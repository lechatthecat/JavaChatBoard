...
public interface BoardRepository extends JpaRepository<Board,Long>{
    ...
    @Query(value = 
          "select * from boards inner join (select br1.board_id, br1.id, br1.updated from (select * from board_responses where board_id in ((select board_id from board_responses where user_id = ?1))) AS br1 "
    ...
    @Query(value = 
          "select count(*) from boards inner join (select br1.board_id, br1.id, br1.updated from (select * from board_responses where board_id in ((select board_id from board_responses where user_id = ?1))) AS br1 "
        + "left join (select * from board_responses where board_id in (select board_id from board_responses where user_id = ?1)) AS br2 ON (br1.board_id = br2.board_id AND br1.updated < br2.updated) where br2.updated is null limit ?2) br " 
        + "on boards.id = br.board_id " 
        + "order by br.updated desc limit ?2", nativeQuery = true)
